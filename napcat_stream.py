"""NapCat Stream API 上传兜底：用于解决 aiocqhttp 大 base64 发送慢的问题。

参考自 astrbot_plugin_gemini_image_generation/tl/napcat_stream.py。

NapCat 提供了 `upload_file_stream` 这个 OneBot v11 扩展 action，
允许通过 bot 与 NapCat 的现有 websocket 连接分块上传文件到 NapCat 服务端，
避免走 `file://` 协议的整文件 base64 序列化路径。
"""
...
DEFAULT_STREAM_CHUNK_SIZE = 64 * 1024  # 64KB
DEFAULT_FILE_RETENTION_MS = 30 * 1000  # 30s
...
async def upload_file_stream(
    ...
    *,
    ...
    """通过 NapCat Stream API 把本地文件上传到 NapCat 服务端，返回 NapCat 文件路径。

    返回 None 表示不支持 / 失败，调用方应回退到普通 file:// 协议。
    """
